...
set siz [array size s]
set count 0

# Finding neighbours of 1st node
for {set i 0} {$i < $siz } { incr i } {
	if {$s($i)==$query && $dist($i)<=500} {
		set nquery($count) $d($i)
		puts "Neighbours of 1st node $query are: $nquery($count):\n"
		set count [expr $count+1]
		
	}
}

set siz2 [array size nquery]
set count2 0

# Finding common neighbours of queried node
for {set i 0} {$i < $siz } { incr i } {
	if {$s($i)==$stat && $dist($i)<=500} {
		set temp  $d($i)
		for {set j 0} {$j < $siz2 } { incr j } {
			if {$temp==$nquery($j)} {
				set nstat($count2) $temp
				puts "Common Neighbours are: $nstat($count2):\n"
...
set flag 0
set siz4 0
set siz3 [array size nstat]
for {set i 0} {$i < $siz3 } { incr i } {
	set n1 $nstat($i)
	set siz4 [array size new_nstat]
	for {set j 0} {$j < $siz4 } { incr j } {
		if {$n1==$new_nstat($j)} {
			set flag 1
			break
		}
	...
	if {$flag==0} {
		set new_nstat($siz4) $n1
	}
	set flag 0
	
}
puts "Common neighbours of $query and $stat are: \n"
for {set j 0} {$j < $siz4 } { incr j } {
	puts "$new_nstat($j)\n"
}
...
set tracefd [open wRecomendation.tr w]
set namtrace [open wRecomendation.nam w]
...
for {set i 0} {$i<$siz4} {incr i} {
set tcp [new Agent/TCP/Newreno]
$ns attach-agent $node($new_nstat($i)) $tcp
...
$ns attach-agent $node($new_nstat($i)) $udp
